...
function createElementWrapper(element, page) {
  return {
     ...
     */
    get id() {
      return element.evaluate(el => el.id);
    },
    ...
    /**
     * Get the classList of the element as an array
     * @returns {Promise<string[]>}
     */
    get classList() {
      return element.evaluate(el => Array.from(el.classList));
    },
     ...
     */
    getAttribute(name) {
      return element.evaluate((el, attrName) => el.getAttribute(attrName), name);
    },
...
export async function type(selector, text, options = {}) {
  const element = await query(selector);
  await element.type(text, options);
}

/**
 * Clear the value of an input element
 * @param {string} selector - CSS selector of the input to clear
 * @returns {Promise<void>}
 */
export async function clear(selector) {
  ...
  // Use evaluate to directly set the value to empty string and focus
  await page.evaluate((sel) => {
    const el = document.querySelector(sel);
    if (el) {
      el.value = '';
      // Trigger events so frameworks can react to the change
      el.dispatchEvent(new Event('input', { bubbles: true }));
      el.dispatchEvent(new Event('change', { bubbles: true }));
      // Focus the element for subsequent operations
      el.focus();
    }
  }, selector);
}
...
export async function setup(options = {}) {
  ...
  try {
    // Use webpackIgnore magic comment to make puppeteer truly optional
